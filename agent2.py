...
def _build_type_variants(text):
	...
	# City-like short form fallback (e.g., Los Angeles -> LA)
	...
	if action_type == "click_text":
		...
		try:
			if _looks_like_url(text):
				...
				# If target is provided (especially destination), focus that field first.
				if target:
		...
		for step in range(25):
			...
			if action_obj.get("action") == "click_text":
				...
				if target_lower in AMBIGUOUS_CLICK_TARGETS:
					...
					action_obj = {
						"action": "scroll",
						"reflection": "Avoid ambiguous click target"
					}
